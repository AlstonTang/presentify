			...
			<main className="flex-1 flex overflow-hidden relative">
				...
				>
					{Array.from({ length: lineCount }).map((_, i) => (
						<div
							key={i}
							className="text-[18px] text-text-dim/90 text-right pr-4"
					...
					{showGuide && (
						...
						>
							<div className="p-6 border-b border-white/5 bg-white/[0.02]">
								...
								<p className="text-[10px] text-text-dim uppercase tracking-widest font-medium">Markdown Reference</p>
							</div>

							<div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
								<section>
									<h4 className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-4">Structure</h4>
									<div className="space-y-3">
										<CodeSnippet code="---" label="Horizontal slide" description="Creates a new main slide." />
										<CodeSnippet code="--" label="Vertical slide" description="Creates a nested sub-slide." />
										<div className="p-3 border border-violet-500/10 bg-violet-500/5 rounded-xl">
											<p className="text-[10px] uppercase font-bold text-violet-400 mb-1">Implicit Splitting</p>
											<p className="text-text-dim text-[11px] leading-relaxed">Slides automatically split at # and ## headers.</p>
										</div>
									</div>
								</section>

								<section>
									<h4 className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-4">Elements</h4>
									<div className="space-y-3">
										<CodeSnippet code="::left" label="Left Alignment" description="Align current slide to the left." />
										<CodeSnippet code="$E=mc^2$" label="Inline Math" description="LaTeX math between single $ icons." />
										<CodeSnippet code="$$ \int x^2 $$" label="Block Math" description="Large math blocks with double $$." />
										<CodeSnippet code="![](image.jpg =500x)" label="Image Sizing" description="Specify width or WxH after URL." />
									</div>
								</section>
								...
								<section className="pb-4">
									<h4 className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-4">Shortcuts</h4>
									<div className="grid grid-cols-2 gap-2">
										{[
											...
											<div key={s.label} className="flex items-center justify-between p-2 bg-white/5 rounded-lg border border-white/5 text-[10px]">
												<span className="text-text-dim">{s.label}</span>
												<kbd className="px-1.5 py-0.5 bg-white/10 rounded font-sans font-bold text-white/70">{s.key}</kbd>
											</div>
										))}
									</div>
								</section>
...
const CodeSnippet: React.FC<{ code: string, label: string, description?: string }> = ({ code, label, description }) => {
	...
	return (
		...
		>
			<div className="flex items-center justify-between w-full mb-1">
				...
				<div className={`transition-all duration-300 ${copied ? 'text-green-400 scale-110' : 'text-white/20 group-hover:text-white/40'}`}>
			...
			<div className="flex flex-col">
				...
				{description && <p className="text-[10px] text-text-dim leading-relaxed mt-0.5">{description}</p>}
			</div>
			{copied && (
				<div className="absolute inset-0 bg-green-500/10 rounded-xl pointer-events-none flex items-center justify-center">
